...
begin
    function input_data(fam)
       # return a vector of ("result_chain_idxxx.jld2", xxx) for every matching folder in datadir("equilibration", fam)
        return if isdir(datadir("equilibration", fam))
            @chain datadir("equilibration", fam) begin
                readdir
                map(s -> match(r"results_chains_id([0-9]+)\.jld2", s), _)
                filter(!isnothing, _)
                map(_) do m
                    (datadir("equilibration", fam, m.match), parse(UInt, m.captures[1]))
                end
            end
        else
            []
    ...
    # datadir("equilibration", fam, "results_chains.jld2")
	output_data(fam, id) = datadir(
	    "equilibration", fam, "hamming_inter_intra_chains_id$(id).jld2"
	)
end
